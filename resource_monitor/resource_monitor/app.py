...
app.config['docker_hostname'] = os.environ.get('docker_hostname', 'socket.gethostname().upper()')#socket.gethostname().upper()
app.config['docker_host'] = os.environ.get('docker_host', 'https://10.0.10.132:2375/')#"https://10.0.10.132:2375/"
# app.config['docker_hostname'] = "10.0.10.132"
app.config['tls_cert_path'] = os.environ.get('tls_cert_path', '/home/svenkatesan/docker_test')
app.config['deh_client_port'] = os.environ.get('deh_client_port', '5003')
...
app.config['secure_connection'] = os.environ.get('secure_connection', "False")
app.config['auto_register'] = "Manual"
app.config['auto_register_rrm'] = False
app.config['auto_register_bse'] = os.environ.get('auto_register_bse', "True")
if app.config['auto_register_bse'].lower().strip() == "true":
    app.config['auto_register_bse'] = True
elif app.config['auto_register_bse'].lower().strip() == "false":
    app.config['auto_register_bse'] = False

# DEH BSE Config
app.config['DEH_BSE_Host'] = os.environ.get('DEH_BSE_Host', 'https://vm1.test.h2020-demeter-cloud.eu')
...
app.config['DEH_BSE_ACS_Token_Request_Url'] = os.environ.get('DEH_BSE_ACS_Token_Request_Url',
                                                             'https://acs.bse.h2020-demeter-cloud.eu:5443/v1/auth/tokens')
app.config['DEH_BSE_Capability_Token_Url'] = os.environ.get('DEH_BSE_Capability_Token_Url',
                                                            'https://acs.bse.h2020-demeter-cloud.eu:3030')
app.config['DEH_BSE_Proxy_URL'] = os.environ.get('DEH_BSE_Proxy_URL', 'https://vm1.test.h2020-demeter-cloud.eu:443')


# Metrics internal DB - MongoDB
# app.config['mongo_host'] = "10.0.10.131"
# app.config['mongo_port'] = 27018
app.config['mongo_host'] = os.environ.get('MONGODB_HOST', "10.0.10.131")
app.config['mongo_port'] = os.environ.get('MONGO_DB_PORT', 27018)
app.config['mongo_db'] = os.environ.get('MONGODB_DATABASE', "DEHClient")
...
app.config['DEHEnablerHub_Host'] = os.environ.get('DEHEnablerHub_Host',
                                                  'https://deh.h2020-demeter-cloud.eu/')
app.config['DEHEnablerHub_Resource'] = '/api/v1/resources'
app.config['DEHEnablerHub_Search_Resource'] = '/api/v1/resources/search'
app.config['DEH_ACCOUNT_MAIL'] = os.environ.get('DEH_ACCOUNT_MAIL', 'sundaresanrocks@gmail.com')
app.config['DEH_ACCOUNT_PASS'] = os.environ.get('DEH_ACCOUNT_PASS', 'Brain@123')
app.config['ACS_Token_Request_Url'] = os.environ.get('ACS_Token_Request_Url',
                                                     'https://acs.bse.h2020-demeter-cloud.eu:5443/v1/auth/tokens')
app.config['Capability_Token_Url'] = os.environ.get('Capability_Token_Url',
                                                    'https://acs.bse.h2020-demeter-cloud.eu:3030')
app.config['DEH_RRM_Proxy_URL'] = os.environ.get('DEH_RRM_Proxy_URL',
                                                 'https://acs.bse.h2020-demeter-cloud.eu:1029')
app.config['DEH_RRM_Search_Resource'] = "/api/v1/resources/search"
app.config['DEH_Save_Resource_Url'] = "/api/v1/resources"
...
app.config['bse_register_service_format'] = {
                        "Service_name": "string",
                        "Tags": "string",
                        "Meta": {
                            "URLOptionalParams": "{}",
