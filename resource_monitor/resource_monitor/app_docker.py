...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'demeterenablerclient')
app.config['docker_host'] = socket.gethostname().upper()
app.config['docker_host'] = os.environ.get('docker_host', "https://10.0.10.132:2376/")
app.config['docker_hostname'] = os.environ.get('docker_hostname', "10.0.10.132")
app.config['tls_cert_path'] = os.environ.get('tls_cert_path', "/home/svenkatesan/docker_test")

# Future Implementation: Interval Config: Options - db & instance
app.config['check_rrm_bse_info_from'] = "db"

app.config['auto_register'] = True
app.config['auto_register_rrm'] = True
...
app.config['DEH_BSE_Token'] = 'ySHXUZjQGDlj4RsxojtRr13rcWi2s5Qfx1O4Yxs9RWAHADzc2Ib9vZgBocWZIf60'
...
# DEH Enabler Hub Config
#app.config['DEHEnablerHub_Host'] = 'https://private-anon-77279d4efe-dehresources.apiary-mock.com'
...
# user-id retrieved from https://acs.bse.h2020-demeter-cloud.eu:5443/v1/auth/tokens, GET Token INFO
app.config['user-id'] = "32194dbf-03de-4ac5-a91b-c959ceb97358"
app.config['DEH_BSE_Request_Header'] = {"content-type": "application/json"}
app.config['DEH_RRM_Request_Header'] = {"content-type": "application/json", "user-id": app.config['user-id']}
