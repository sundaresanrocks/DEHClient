        ...
        if event['Action'] == 'die' \
            ...
            # Container Create
                ...
                if app.config['auto_register'] or app.config['auto_register_rrm']:
                    ...
                    # rrm_register = self.deh_api_wrapper_client.deh_rrm_check_resource_registration(service_name)
                    # GET DEH ID
                    print("Future implementation")
                elif not app.config['auto_register_rrm']:
                    self.__logger.info("Skipping the auto registration for RRM as flag auto_register_rrm"
                                       " is set to False")
            # Container went up
                ...
                if event['Action'] == 'start':
                    ...
                    if app.config['auto_register'] or app.config['auto_register_bse']:
                        ...
                        #bse_register = self.deh_api_wrapper_client.deh_bse_check_resource_registration(service_name)
                        print("Future implementation")
                    ...
                    metrics_handler_client = MetricHandler(self.docker_ca_cert, self.docker_client_cert,
                                                                        ...
                                                                        self.container_name)
                    individual_metric = metrics_handler_client.get_metrics_by_container({"name": service_name})
                    ...
                    deh_bse_obj = DEHAPIWrapper(host, method,
                                                payload={"service_name": event['Actor']['Attributes']['name']})
                    ...
                    if len(contents) == 0:
                        self.__logger.info("Service {} not registered to DEH Enabler Hub RRM, "
                                           "Now attempt to register.".format(service_name))
                        '''
                        resource_data = {}
                        try:
                        ...
                        '''
                        resource_data = {'name': service_name}
                        deh_enabler_hub_obj = DEHAPIWrapper()
                        ...
                        if status_code == 200:
                            self.__logger.info("Successfully registered resource: {} "
                                               ...
                                               .format(service_name, response.text))
                        else:
            ...
            documents = self.mongo_client.read({'_id': formatted_data['_id']})

            latest_event = formatted_data['events'][0]
